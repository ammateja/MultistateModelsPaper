...
function summarize_paths_full(paths, model; return_counts = false)
    ...
    counts = @chain dat begin
        ...
        @combine(:N = sum(:weight),
                 ...
                 :a = mean(:asymptomatic),
                 :sero = mean(:seropos),
                 :serosym = mean(:serosym),
                 :seroasym = mean(:seroasym),
...
function fit_collapsed(dat; model_number, SamplingWeights = nothing)
    ...
    # fit models
    model_fitted = fit(model_fit; verbose = true, compute_vcov = false, ess_target_initial = 50, α = 0.2, γ = 0.2, tol = 0.01)
...
function work_function(;seed1, seed2, model_number, nulleff)
    ...
    if model_number <= 2
        ...
        elseif model_number == 2
            initialize_parameters!(model_fit)
            model_fitted = fit(model_fit; verbose = true, compute_vcov = true, maxiter = 500, ess_target_initial = 100, α = 0.2, γ = 0.2, tol = 0.01)
        end
