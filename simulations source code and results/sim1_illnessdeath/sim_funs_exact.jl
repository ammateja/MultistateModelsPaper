...
function setup_model(; make_pars, data = nothing, nsubj = 250, family = "wei", ntimes = 6, spknots = nothing, sim_exp = false)
...
function get_estimates(paths)
    ...
    time2prog_all = mean(map(x -> x.states[2] == 2 ? x.times[2] : 1.0, paths))

    # time to disease among the progressors
    proginds = map(x -> (2 ∈ x.states) && (length(x.states) != 2), paths)
    time2prog_gprog = mapreduce(x -> x.times[2], +, paths[proginds]; init = 0.0) / sum(proginds)
...
function work_function(;simnum, seed, family, ntimes, sims_per_subj, nboot)
    ...
    # model_sim = setup_model(; make_pars = true, data = nothing, family = "exp", ntimes = ntimes, nsubj = 250, sim_exp = true)
        
    # simulate paths
    dat = simulate(model_sim; nsim = 1, paths = false, data = true)
    dat = dat[1]
...
function summarize_crude(paths, dat, model)
    ...
    # time to progression- subdistribution
    times2prog = map(p -> p.states[2] == 2 ? p.times[2] : 1.0, paths)
    prog_status = map(p -> p.states[2] == 2 ? 1 : 0, paths)
    ...
    time2prog_gprog = rcopy(R"sm = survival:::survmean(survfit(Surv($progtimes, $(ones(length(progtimes)))) ~ 1), rmean = max($times))[[1]][c('rmean', 'se(rmean)')];c(est = sm[1], lower = sm[1] - 1.96 * sm[2], upper = sm[1] + 1.96 * sm[2])")
    
    time2prog_all = rcopy(R"sm = survival:::survmean(survfit(Surv($times2prog, $prog_status) ~ 1), rmean = max($times2prog))[[1]][c('rmean', 'se(rmean)')];c(est = sm[1], lower = sm[1] - 1.96 * sm[2], upper = sm[1] + 1.96 * sm[2])")
    ...
    ests_crude = DataFrame(
        ests = [events[1:4]; rmst[1]; time2prog_gprog[1]; time2prog_all[1]; illnessdur[1]],
        lower = [cis.lower; rmst[2]; time2prog_gprog[2]; time2prog_all[2]; illnessdur[2]],
        upper = [cis.upper; rmst[3]; time2prog_gprog[3]; time2prog_all[3]; illnessdur[3]])
end
...
function crude_ests(;seed, ntimes)
    ...
    return hcat(DataFrame(simnum = seed, 
                     ntimes = ntimes,
                     family = "crude",
                     var = ["pfs", "prog", "die_wprog", "die_noprog", "rmpfst", "time2prog_gprog", "time2prog_all", "illnessdur"]),
                     ests_crude)
end
